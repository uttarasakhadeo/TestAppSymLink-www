...
$(document).on("pageshow", "#designpage", function(){	
	if(solvingForValue){		
		...
		if($("#smallestgroupsize_row")){
			$('#smallestgroupsize_row').remove();
		}		
		if($("#power_row")){
			$('#power_row').remove();
		}
		...
		if(solvingForValue == "Power"){												
			$('#input_listview').append('<li data-theme="b" id="smallestgroupsize_row"><a href="samplesize.html" data-transition="flip">'+
				'<h2 id="smallestgroupsize-header">Smallest Group Size <span id="smallestgroupsize-list-option" class="ui-li-count"><img src="../images/incomplete_icon.png"/>'+
	...
	/*if(numberOfGroupsValue){
		$('#numberofgroups-list-option').html("Complete");						// Displaying selected value in header line
	}
	else{
		if($('#numberofgroups-list-option').is(':contains("Complete")')){		
			$('#numberofgroups-list-option').empty();		
			$('#numberofgroups-list-option').append('<img src="../images/incomplete_icon.png">').trigger('create');			
			$('#input_listview').listview('refresh');		
		}
	}*/
});
